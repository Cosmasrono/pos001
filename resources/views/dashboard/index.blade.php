            ...
            <div class="d-flex align-items-center gap-2">
                ...
                <span>
                    ...
                    @if($trialDaysRemaining !== null)
                        &mdash; {{ $trialDaysRemaining }} {{ $trialDaysRemaining === 1 ? 'day' : 'days' }} remaining
                    @endif
                    @if($subscriptionExpiresAt)
                        &nbsp;·&nbsp; ends {{ $subscriptionExpiresAt->format('M d, Y') }}
                    @endif
                </span>
            </div>
    ...
    @elseif($subscriptionStatus === 'active')
        <div class="sub-banner active">
            <div class="d-flex align-items-center gap-2">
                ...
                <span>Subscription: <span class="text-uppercase">ACTIVE</span>
                    @if($subscriptionExpiresAt)
                        &nbsp;·&nbsp; Renews {{ $subscriptionExpiresAt->format('M d, Y') }} ({{ $subscriptionExpiresAt->diffForHumans() }})
                    @endif
                </span>
            </div>
...
{{-- ── AI DAILY BRIEF ── --}}
@if(isset($aiBrief) && $aiBrief)
<div class="card border-0 shadow-sm mb-3" style="background: linear-gradient(135deg, #f5f3ff 0%, #ede9fe 100%); border-left: 4px solid #7c3aed !important;">
    <div class="card-body p-3">
        <div class="d-flex align-items-start gap-3">
            <div style="width:42px; height:42px; border-radius:12px; background:rgba(124,58,237,.15); display:flex; align-items:center; justify-content:center; flex-shrink:0;">
                <i class="bi bi-stars" style="color:#7c3aed; font-size:1.2rem;"></i>
            </div>
            <div class="flex-grow-1">
                <div class="d-flex align-items-center justify-content-between mb-2">
                    <h6 class="mb-0 fw-bold" style="color:#5b21b6;">Today's AI Business Brief</h6>
                    <small class="text-muted">{{ $aiBrief->brief_date->format('D, j M') }}</small>
                </div>
                <div style="font-size:.9rem; line-height:1.6; color:#1e1b4b; white-space:pre-line;">{{ $aiBrief->content }}</div>
                <div class="mt-2 d-flex align-items-center gap-2" style="font-size:.72rem; color:#6b7280;">
                    <i class="bi bi-info-circle"></i>
                    <span>Generated by AI · Refreshed daily</span>
                </div>
            </div>
        </div>
    </div>
</div>
@elseif(auth()->user()->isOwner() && auth()->user()->company)
<div class="card border-0 shadow-sm mb-3" style="background:#f8fafc; border-left:4px solid #94a3b8 !important;">
    <div class="card-body p-3">
        <div class="d-flex align-items-center gap-3">
            <i class="bi bi-stars text-muted" style="font-size:1.5rem;"></i>
            <div>
                <strong>AI Brief unavailable today</strong>
                <div class="small text-muted">Will retry tomorrow. Check that GROQ_API_KEY is configured.</div>
            </div>
        </div>
    </div>
</div>
